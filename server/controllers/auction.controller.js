...
const auctionByID = async (req,res,next,id) => {
    try{
        const auction = await Auction.findById(id)
                         ...
                         .populate('bids.bidders','_id name').exec()
        if(!auction){
            return res.status(400).json({
...
const isSeller = (req,res,next) => {
    const isSeller = req.auction?.seller._id && req.auth?._id
...
export default {
    create,auctionByID,getPhoto,listBySeller,update,remove,
    defaultPhoto,listOpen,listByBidder,isSeller,read
}