            ...
            <span>
                 ...
                 {currentDate > new Date(auction.bidStart)
                  && currentDate < new Date(auction.bidEnd)
                   && <>{`Auction is live | ${auction.bids.length}
                    bids |`} {showTimeLeft(new Date(auction.bidEnd))}</>
                    }
                 {currentDate > new Date(auction.bidEnd)
                  && `Auction Ended | ${auction.bids.length}
                   bids`
                   } 
                 {currentDate > new Date(auction.bidStart)
        ...
        <List dense>
            {props.auctions.map((auction,i) => (
                <span key={i}>
                    <ListItem button >
                        <ListItemAvatar>
                            <Avatar variant="squared" src={`/api/auctions/image/${auction._id}?${new Date().getTime()}`} />
                        </ListItemAvatar>
